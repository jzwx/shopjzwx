...
<script type="text/javascript">
	...
	function delAdmin(id){
		if(confirm("确认删除吗?"))
		{
			window.location="delUser.bin?id="+id;
			return ;
		}
		else
		{
 ...
 <div class="details">
                    ...
                    int n=10;
                    	...
                    	String sql="select * from jzwx_user";
                    ...
                    <table class="table" cellspacing="0" cellpadding="0">
                        ...
                        <%
                         ...
                         <div align="center">
                    <a href="listUser.jsp?page=1">[首页]</a>
                    <%
                    	...
                    	if(nowPage>1){
                     %>
                    <a href="listUser.jsp?page=<%=nowPage-1 %>">[上一页]</a>
                    <%
                    }
                    else
                    {
                    	%>
                    	<a href="listUser.jsp?page=1">[上一页]</a>
                    	<%
                    }
                    for(int i=1;i<=pageCount;i++)
                    {
                    	%>
                    		<a href="listUser.jsp?page=<%=i%>">[<%=i %>]</a>
                    	<%
                    }
                    if(nowPage<pageCount){
                    %>
                          <a href="listUser.jsp?page=<%=nowPage+1 %>">[下一页]</a>
                    <%
                    }
                    else{
                    	%>
                    	<a href="listUser.jsp?page=<%=pageCount %>">[下一页]</a>
                    	<%
                    }
                    %>
                    <a href="listUser.jsp?page=<%=pageCount %>">[尾页]</a><span style="float: right;">总共<%=pageCount %>页/当前是第<%=nowPage %>页</span>
                    <%
                    }catch(Exception e)
                    {
                    	e.printStackTrace();
